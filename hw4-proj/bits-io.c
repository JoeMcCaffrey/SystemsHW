...
if(bfile->nbits == -1){ // check to see if this is the first time we read the file

bfile->nbytes= 0 ; // initialise this as instructed
bfile->nbits= 0 ;
//printf("HHHHHHH");

//bfile-> nbytes++; // we are reading a byte calling this function so increment
bfile->byte = fgetc(bfile->fp) ;// correct

 // throw all bytes into temp byte buffer
// increment a change in bits
}
...
// push the rest of the bytes into the buffer


unsigned char r;// temp buffer
unsigned char j = 0 ;
unsigned char k = 0;

//printf("bfile->byte before stuff %d\n",bfile->byte);
//printf("bfile->nbits before stuff %d\n",bfile->nbits);
if(bfile->nbits == 0){

j= bfile->byte;
k=j>>7;
//printf("should be most sig %d\n",k);
...
bfile->byte = bfile->byte<<1 ;
j=bfile->byte ;
//printf("J value in while %d\n",j);
bfile->nbits++;
k= j>>7;
//printf("most sig in loop %d\n");
//printf("bfile in loop %d\n", bfile->byte);
}
}
