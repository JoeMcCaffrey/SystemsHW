...
#if 0
printf("ctx->table->v = %d\n", (int *) ctx->table->v);
printf("char thingy %u\n", getc(fp));
printf("char thingy 2 %u\n",getc(fp));
#endif

unsigned int in =getc(fp);
while(in != EOF){

if (in <= CHAR_END){
ctx->table[in-CHAR_START].v++;
ctx->table[in-CHAR_START].c = in;


#if 0
printf("table value1%d\n",ctx->table->v);

printf("table1 value %d\n",ctx->table->c);
exit(1);
#endif
}
in=getc(fp);
...
temp->freq=ctx->table[i];// save the char and count into freqency freq
...
TreeNode* root = tree_new();
root->freq.v = F;
root->left = L;
root->right = R;
root->type = INTERNAL;
pqueue_enqueue(ctx->pq, root);
//tree_free(L);
//tree_free(R); //changed
}
TreeNode * returned ;
returned = pqueue_dequeue(ctx->pq);
  return returned;
}
