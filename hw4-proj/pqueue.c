...
static void sort(PriorityQueue *pq) {
  qsort(pq->queue, pq->next+1, sizeof(TreeNode*), comparator);
}
...
#if 0
printf("return i = %x\n", i);
printf("return pq->queue[i] = %x\n", (unsigned long)pq->queue[i]);
printf("return pq->queue[i+1] = %x\n", (unsigned long)pq->queue[i+1]);
#endif
